---
name: task
description: Clarify the next project task and prepare context. Use only when the workflow skill routes the active session to task.
---
...
5. Update `docs/project/NEXT_STEP_CONTEXT.md`.
...
Return the phase result to `workflow`; do not edit `WORKFLOW.md` or start `requirements`. Recommend `requirements` when passed, or `task` when blocked or revision is needed.
