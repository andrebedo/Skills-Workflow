---
name: review
description: Review a green slice and publish validated, portfolio-ready architecture. Use only when the workflow skill routes the active session to review.
---
...
4. Create or update `docs/project/ARCHITECTURE.md` in English.
...
6. Validate every Mermaid diagram.
   Require `mmdc`, installed by `setup` as `@mermaid-js/mermaid-cli`. Extract each Mermaid block to temporary files, render it to temporary SVG, and delete temporary artifacts after validation. Do not commit generated SVGs. A missing renderer or rendering error blocks review and is recorded with recovery guidance.
...
Return the phase result to `workflow`; do not edit `WORKFLOW.md`. Recommend session closure when passed, or the evidenced corrective phase otherwise.
